...
-- === Sampling tuning (custom) ===
-- These control per-sample resource yield and cadence. Read by
-- ResourceManagerImplementation::loadConfigData() and pushed into
-- ResourceSpawner via setSampleTuning(). Lua-only edits take effect on
-- `docker-compose restart swgemu`.
sampleYieldMultiplier = 5.0          -- flat multiplier on per-sample yield
sampleIntervalMs = 5000             -- ms between consecutive /sample ticks
sampleGambleMultiplier = 5.0         -- payout on successful gamble roll
sampleConcentrationMultiplier = 5.0  -- payout within 10m of a rich node
